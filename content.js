if (typeof window.panelZoomInjected === 'undefined') {
    ...
    function injectFAB() {
        if (document.getElementById('panpan-fab-container')) return; // Already injected
        
        fabElement = document.createElement('div');
        fabElement.id = 'panpan-fab-container';
        
        // Using Shadow DOM to prevent kagane.to CSS from bleeding into the FAB
        ...
        button.innerHTML = '📖'; // Placeholder icon
        
        // Launch the reader overlay on click
        button.addEventListener('click', () => {
            chrome.storage.local.get('savedSettings', (data) => {
                const settings = data.savedSettings || {};
                viewerSettings = settings;
                if (!viewerSettings.zoomMode) viewerSettings.zoomMode = 'dynamic';
                pzViewerSettings = viewerSettings;
                if (typeof overlayHost !== 'undefined' && overlayHost) {
                    if (typeof applySettingsUpdate === 'function') applySettingsUpdate();
                } else {
    ...
    // Listen for real-time toggle events from the popup
